...
Horizontal Pod Autoscaler automatically increases or decreases the number of Pod replicas based on resource utilization or other supported metrics.

---

## What is the difference between HPA and manually scaling a Deployment?

**Answer:**

Manual scaling requires the administrator to change the replica count. HPA automatically adjusts the replica count based on the configured metrics and targets.

---

## What does `minReplicas` and `maxReplicas` define in HPA?

**Answer:**

`minReplicas` defines the minimum number of Pods HPA can maintain, while `maxReplicas` defines the maximum number of Pods HPA can create.

---

## What does `averageUtilization: 60` mean in a CPU-based HPA?

**Answer:**

HPA attempts to maintain the average CPU utilization of the Pods at approximately 60% of their requested CPU resources.

---

## Which API version is commonly used for modern HPA configuration?

**Answer:**

`autoscaling/v2` is commonly used because it supports multiple metric types and more advanced scaling configuration.

---

## Why is a load generator useful when testing HPA?

**Answer:**

A load generator creates application traffic that can increase resource utilization. This allows us to observe whether HPA detects increased utilization and automatically increases the number of Pod replicas.

---

## Does sending more HTTP requests always cause HPA to scale?

**Answer:**

No.

Scaling depends on the resource utilization generated by the requests, the HPA target, available resources, and the HPA evaluation behavior. A large number of lightweight requests may not generate enough CPU utilization to trigger scaling.

---

## How do you verify that HPA is scaling Pods during a load test?
...
and monitor the Pods:

```bash
kubectl get pods -n <namespace> -w
```

You can also check CPU usage with:
...
## Why should a load generator be reusable instead of being tied to one service?

**Answer:**

A generic load generator can be used to test different services and endpoints without creating separate testing tools for each microservice.

---

## What happens when the CPU utilization of an HPA target increases above its configured target?

**Answer:**

HPA calculates the required replica count based on the observed CPU utilization and target utilization. If additional replicas are required, Kubernetes increases the Deployment's replica count.

---

## Why doesn't HPA immediately scale down when traffic stops?

**Answer:**

HPA uses stabilization and evaluation behavior to avoid rapidly increasing and decreasing replicas. This prevents unnecessary scaling caused by short-lived changes in workload.

---

## How do you verify HPA scale-up and scale-down?
...
## What is the difference between the current replicas and desired replicas in HPA?

**Answer:**

**Current replicas** are the number of Pods currently running, while **desired replicas** are the number of Pods HPA calculates should be running based on the current metrics and configuration.
...
HPA changes the number of Pod replicas based on resource utilization or other metrics, while VPA adjusts or recommends CPU and memory resource values for Pods.

---

## What does VPA `updateMode: Off` mean?

**Answer:**

`Off` means VPA only provides resource recommendations. It does not automatically modify the resource requests of running Pods.

---

## What kind of resources does VPA manage?

**Answer:**

VPA primarily manages CPU and memory resource requests and limits for containers.

---

## Why use VPA in recommendation mode for this project?

**Answer:**

Recommendation mode allows us to observe VPA's CPU and memory recommendations without automatically restarting or modifying application Pods.
...
Yes, but they should be configured carefully. HPA can handle replica scaling while VPA can handle resource recommendations. Using both to control the same resource dimension can cause conflicting scaling behavior.
...
A ServiceAccount provides an identity for Pods when they need to interact with the Kubernetes API.
...
A Role provides permissions within a specific namespace, while a ClusterRole can provide permissions at the cluster level or be used across namespaces.

---

## Why did you use a Role instead of a ClusterRole in your project?

**Answer:**

The application only requires namespace-level access, so a Role is sufficient. Using a ClusterRole would provide unnecessary permissions and violate the principle of least privilege.

---

## Why did you give the application ServiceAccount no Kubernetes API permissions?

**Answer:**

The application does not need to communicate with the Kubernetes API. Therefore, giving it Kubernetes API permissions would be unnecessary and increase the security risk.
...
The principle of least privilege means giving an application or user only the permissions required to perform its intended tasks and no additional permissions.
...
A NetworkPolicy controls network traffic between Pods and other network endpoints. It can restrict incoming traffic, outgoing traffic, or both.

---

## What is the difference between RBAC and NetworkPolicy?

**Answer:**

RBAC controls what users, ServiceAccounts, and other identities can do through the Kubernetes API.

NetworkPolicy controls which network connections are allowed between Pods and other endpoints.
...
`podSelector` determines which Pods the NetworkPolicy applies to. For example:
...
applies the policy to Pods labeled `app=product-service`.
...
Ingress controls incoming network traffic to selected Pods, while Egress controls outgoing network traffic from selected Pods.

---

## Why shouldn't you allow all network traffic in a production Kubernetes environment?

**Answer:**

Allowing unrestricted traffic increases the attack surface. NetworkPolicies can restrict communication so that services can only communicate with the resources they actually need.
...
Running as a non-root user reduces the impact of a container compromise because the application does not have root privileges inside the container.

---

## What does `runAsNonRoot: true` do?

**Answer:**

It tells Kubernetes that the container must run as a non-root user. If the container attempts to run as UID 0, Kubernetes prevents it from starting.
...
It prevents processes inside the container from gaining additional privileges beyond those they were started with.

---

## Why do you drop Linux capabilities from the container?

**Answer:**

Linux capabilities provide additional privileges to processes. Since the Node.js application does not require them, dropping all capabilities reduces the container's attack surface.
...
It prevents processes from modifying the container's root filesystem, reducing the ability of a compromised application to write malicious files or modify the container environment.

---

## How do you verify which ServiceAccount a Pod is using?
...
The output shows the ServiceAccount assigned to the Pod.

---

## How do you verify that a ServiceAccount cannot access Kubernetes resources?
...
```bash
kubectl auth can-i --as=system:serviceaccount:<namespace>:<serviceaccount-name> get pods -n <namespace>
```

If the result is `no`, the ServiceAccount does not have that permission.
...
The UID should not be `0`.

---

## Why should security validation include application functionality checks?

**Answer:**

Security restrictions can prevent legitimate application behavior, such as writing temporary files or accessing required resources. Functional checks confirm that the application remains usable after security changes.

---

## What should be checked after enabling `readOnlyRootFilesystem`?

**Answer:**

Check the application logs and Pod status for filesystem permission errors. If the application needs temporary writable storage, provide a dedicated writable volume instead of making the entire root filesystem writable.
